    ...
    def _train_epoch(self, dataloader):
        ...
        for i, batch in enumerate(dataloader):
            ...
            with torch.autocast(device_type=self.device, dtype=torch.float16):
                ...
                # print(seq_out.shape, targets_golden.shape)
                ce_loss = self.ce_criterion(seq_out.view(-1, seq_out.size(-1)), targets_golden.view(-1))
                
                
                # TODO: Calculate CTC loss if needed
                if self.ctc_weight > 0:
                    ctc_loss = self.ctc_criterion(ctc_inputs['log_probs'], targets_golden, torch.tensor(ctc_inputs['lengths'], dtype=torch.long, device=feats.device), transcript_lengths)
